...
export type SongLyricSourceType =
	| "embedded"
	| LyricSourceType
	| "none"
	| "unknown";
...
export interface SongLyricSourceInfo {
	type: SongLyricSourceType;
	id?: string;
	name?: string;
}
...
export function guessLyricFormat(
	...
	if (/\[\d+,\d+\]/.test(trimmed) && /\(\d+,\d+\)/.test(trimmed)) return "qrc";
	// LYS：行首 [属性数字] 后接 ((start,dur)) 词块
