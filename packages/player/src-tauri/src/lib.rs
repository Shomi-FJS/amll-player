...
async fn pick_directory_tree_uri() -> Result<Option<String>, String> {
    tokio::task::spawn_blocking(|| -> Result<Option<String>, String> {
        ...
        // 调用 Kotlin 侧的 MainActivity.pickDirectoryTree()，签名为 () -> String?
        let result = env
            .call_static_method(
                &main_class,
                "pickDirectoryTree",
...
fn document_id_to_fs_path(doc_id: &str) -> Option<String> {
    ...
    let base = if volume == "primary" {
        ...
        // 次存储（SD 卡 / OTG 等）在 Android 里都挂载在 `/storage/<卷标>` 下。
...
/// 递归扫描 SAF 目录树，返回 `/storage/...` 真实路径列表。
/// 需要 `MANAGE_EXTERNAL_STORAGE` 权限。无法映射真实路径的条目（如云盘 Provider）静默跳过。
///
/// `recursive`：false 只扫选中目录本层；true 递归所有子文件夹。
#[cfg(target_os = "android")]
#[tauri::command]
async fn scan_audio_in_tree_uri(tree_uri: String, recursive: bool) -> Result<Vec<String>, String> {
    ...
    tokio::task::spawn_blocking(move || -> Result<Vec<String>, String> {
        ...
        // Uri.parse(treeUri) —— 把字符串包成 android.net.Uri 对象
        ...
        // 拿到 ContentResolver，后面所有针对 SAF 树的查询都通过它进行
        let resolver = env
            .call_method(
                &context,
                "getContentResolver",
        ...
        while let Some(doc_id) = stack.pop() {
            ...
            let projection: JObjectArray =
                match env.new_object_array(3, &str_class, JObject::null()) {
                    Ok(v) => v,
                    Err(_) => return Ok(()),
                };
