...
async fn query_content_display_name(uri: String) -> Result<Option<String>, String> {
    tokio::task::spawn_blocking(move || -> Result<Option<String>, String> {
        ...
        let query_result = (|| -> Result<Option<String>, String> {
            let has_row = env
        ...
        let close_result = env
            .call_method(&cursor, "close", "()V", &[])
            .map(|_| ())
            .map_err(|e| format!("cursor close failed: {e}"));
...
async fn scan_audio_in_tree_uri(tree_uri: String, recursive: bool) -> Result<Vec<String>, String> {
    ...
    info!("scan_audio_in_tree_uri START tree_uri={tree_uri} recursive={recursive}");
    tokio::task::spawn_blocking(move || -> Result<Vec<String>, String> {
        ...
        let mut last_log_files = 0usize;
        while let Some(doc_id) = stack.pop() {
            if SCAN_CANCEL.load(Ordering::SeqCst) {
                canceled = true;
                break;
            }
            // 卡住时 logcat 上能直接看到最后一个 doc_id，方便定位。
            debug!("scan dir doc_id={doc_id} (depth_remaining={})", stack.len());
            ...
            dirs_done += 1;
            // 每 32 个目录或新增 100 个文件打一行进度，方便看「卡在哪」。
            if dirs_done % 32 == 0 || results.len() - last_log_files >= 100 {
                info!(
                    "scan progress: dirs_done={dirs_done} files={} stack_remaining={}",
                    results.len(),
                    stack.len()
                );
                last_log_files = results.len();
            }
        }
...
#[cfg(not(target_os = "android"))]
#[tauri::command]
async fn scan_audio_in_tree_uri(
    _tree_uri: String,
    _recursive: bool,
...
async fn read_local_music_metadata(
    ...
    let audio_info = tokio::task::spawn_blocking(move || -> anyhow::Result<AudioInfo> {
        // 大量 mp3/flac 内嵌封面图（mjpeg/png）会被 FFmpeg 当 video stream 深度
        // probe，每首多花 30~80ms。我们只读元数据，不需要识别封面解码参数，
        // 把 probesize/analyzeduration 压到极小可大幅加速首次导入。
        let mut opts = ffmpeg::Dictionary::new();
        opts.set("probesize", "32768");
        opts.set("analyzeduration", "0");
        let mut input_ctx = ffmpeg::format::input_with_dictionary(&path_clone, opts)
    ...
    {
        // TTML > LyS > YRC > QRC > ESLRC > LRC
        const LYRIC_FILE_EXTENSIONS: &[&str] = &["ttml", "lys", "yrc", "qrc", "eslrc", "lrc"];
        // Batch read_dir to avoid 6 exists() calls on FUSE mounts
