...
impl HttpServerController {
    ...
    async fn start(&mut self) {
        ...
        self.server_handle = Some(tokio::spawn(async move {
            run_http_server(app, dist_dir, listener, shutdown_rx).await;
        }));
    }
...
async fn api_ws_listen_addr(
    ...
    // 通知前端更新 WS 监听地址 atom，前端会用持有的 Channel 重新发起 reopen
    if let Err(err) = state.app.emit("remote-set-ws-listen-addr", &addr) {
        error!("Failed to emit remote-set-ws-listen-addr: {:?}", err);
...
pub fn build_router(app: AppHandle, dist_dir: PathBuf) -> Router {
    let (song_events, _) = broadcast::channel(64);
    let state = HttpServerState { app, song_events };
    Router::new()
...
async fn run_http_server(
    ...
    let router = build_router(app, dist_dir);
